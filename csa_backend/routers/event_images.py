...
def sync_event_images_from_drive(event_title: str, admin_email: str = None) -> dict:
    ...
    try:
        ...
        if not images:
            ...
            return {"success": True, "message": "No images found", "synced_count": 0}
        ...
        try:
            ...
            try:
                db_images = supabase.table('gallery_images').select('id, original_filename, image_url, filename').eq('folder_name', event_title).execute()
                ...
                if db_images.data:
                    for db_image in db_images.data:
                        ...
                        db_file_id = None
                        if db_url and 'drive.google.com' in db_url and 'id=' in db_url:
                            try:
                        ...
                        if not image_exists:
                            ...
                            try:
                                supabase.table('gallery_images').delete().eq('id', db_image['id']).execute()
                                ...
                                logger.warning(f"Failed to delete image {db_image.get('id')}: {delete_error}")
            except Exception as e:
                logger.warning(f"Could not check for deleted images: {e}")
        except Exception as e:
            logger.warning(f"Error detecting deleted images: {e}")
